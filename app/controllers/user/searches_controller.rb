class User::SearchesController < ApplicationController
  def search
    @keyword = params[:word]
    redirect_to request.referer, notice: "検索文字を入れて下さい" if @keyword.blank? # キーワードが入力されていないとトップページに飛ぶ
    ...
    split_keyword.each do |keyword| # 分割したキーワードごとに検索
      ...
      posts += Post.where('title LIKE(?) OR body LIKE(?)', "%#{keyword}%", "%#{keyword}%").order(created_at: :desc)
    end
